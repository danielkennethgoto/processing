/*class Agent_Child {
  ...
  Agent_Child(int x, int y) { // constructor, code to initialize this agent when created
    posX = x; // default to create each new agent where the user's mouse is located
    posY = y;
  ...
  void eatStuffCheck() { // are we close enough to any food to eat it?
    ...
    for(int i=foods.size()-1; i>=0; i--) { // go through each food
      ...
      if(dist(oneFood.posX,oneFood.posY, posX, posY) < 10.0) { // within 10 pixels of it?
        ...
        if (millis_NextNeedForFood>3000){
          have_Child = 1;
        }
  ...
  int drawAndMove() { // called on every agent every frame; run logic and draw on screen
